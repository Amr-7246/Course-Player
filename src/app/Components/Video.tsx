...
const Video = () => {
  ...
  // * ################################# End Logic
  return (
    <div className={` ${isPlaying ? "sticky bg-whiterounded-lg duration-500 bg-white top-0" : "relative"} md:mb-10 w-full min-h-[400px] p-5 flex flex-wrap items-center justify-center gap-2  md:col-span-2 row-span-1 row-start-1 row-end-2  `}>
      {/* Video Section */}
        <div className={` group block w-full h-fit md:max-h-[450px] bg-black rounded-md overflow-hidden relative  ${isRotated ? "transform rotate-90" : ""} `}>
          {/* Video itself */}
            <video ref={videoRef} className="w-full h-full object-cover" muted playsInline onClick={handleTogglePlay} controls={isFullScreen && isSmallScreen} >
              <source
          ...
          {/* Render custom controls only when not in fullscreen on small screens */}
            {( true ) && (
              <>
                ...
                {/* Progress Bar */}
                {/* Expand Button */}
                <button onClick={handleExpand} className=" md:hidden cursor-pointer absolute bottom-4 right-4 bg-gray-800 bg-opacity-70 text-white p-2 rounded-full hover:bg-opacity-90 transition" > <FaExpand /></button>
                {/* Expand Button */}
              </>
            )}
          {/* Render custom controls only when not in fullscreen on small screens */}
