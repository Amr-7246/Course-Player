...
const Video = () => {
  ...
  // * End Logic
  return (
    <div className= {` ${ isPlaying ? 'sticky' : '' } w-full h-[400px] p-5 flex flex-wrap items-center justify-center gap-2 md:col-span-2  row-span-1 row-start-1 row-end-2`}>
      {/* Video Section */}
      <div className={` block w-full h-[80%] bg-black rounded-md overflow-hidden relative top-0 ${ isRotated ? "transform rotate-90" : "" }`}>
        {/* Video itself */}
        ...
        {/*Play/Pause Button */}
          <button onClick={handleTogglePlay} className="absolute inset-0 flex items-center justify-center text-white text-6xl transition-opacity duration-300" >
            {isPlaying ? (
              <FaPause className="opacity-75 hover:opacity-100" />
            ) : (
              <FaPlay className="opacity-75 hover:opacity-100" />
            )}
          </button>
        {/*Play/Pause Button */}
        ...
        {/* Custom Gradient Progress Bar */}
        {/* Expand Button */}
          <button onClick={handleExpand} className="absolute bottom-4 right-4 bg-gray-800 bg-opacity-70 text-white p-2 rounded-full hover:bg-opacity-90 transition">
            <FaExpand />
          </button>
        {/* Expand Button */}
      </div>
